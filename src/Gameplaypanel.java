...
public class Gameplaypanel extends JPanel implements ActionListener, KeyListener {
	...
	public Gameplaypanel() {
		...
		time = new Timer (delay, this);  //(speed, context)
		time.start();
	}
	...
	public void paint (Graphics g) {
		...
		//the ball 
		g.setColor(Color.yellow);
		g.fillOval(ballposX, ballposY, 20, 20); //variables to enable changing with action performed
	...
	public void keyPressed(KeyEvent e) {
		if (e.getKeyCode() == KeyEvent.VK_RIGHT) {
			if(playerX >=600) {   //check if does not go outside the panel
				playerX=600;
			} else {
				moveright();
			}
